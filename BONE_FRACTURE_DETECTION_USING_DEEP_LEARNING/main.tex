...
4.4 \hspace{5pt} Proposed System Architecture and Model Performance Analysis \dotfill 28 \\
4.5 \hspace{5pt} User Interface Dashboard showing Bone Fracture Detection Result \dotfill 30 \\
...
\section*{Methodology}
\justifying
FractureAI utilizes a hybrid architecture where the Frontend (React) manages user interaction, while the Backend (Django) handles image processing and orchestrates the AI Engine (TensorFlow/PyTorch). The system implements a secure RESTful API to ensure data integrity and seamless communication between the diagnostic tools and the medical personnel.

The core of the system is a Vision Transformer (ViT) model trained on large-scale musculoskeletal datasets. Unlike traditional CNNs, ViTs divide images into patches and process them as sequences, allowing the model to capture global contextual information that is critical for identifying subtle fractures in complex skeletal structures.
...
The system employs a Vision Transformer (ViT-16) architecture:
\begin{enumerate}
